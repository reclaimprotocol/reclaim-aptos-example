# Reclaim Protocol Starter Pack - Aptos

This repository provides a **React-based frontend example** for developers to interact with the Reclaim Protocol smart contract on the **Aptos** network. This template demonstrates how to submit and verify proofs on-chain, making it easier for developers to integrate Reclaim Protocol into their applications.
...
If you deployed a custom contract, update the contract address and the owner address in `src/verify-proof.tsx`:

```javascript
const reclaimAddress = "YOUR_CONTRACT_ADDRESS"; // Replace with your contract address
const ownerAddress = "YOUR-OWNER-ADDRESS" 
```

### 5. Switch network

The current default network is TESTNET if you want to change it you can change it from `src/verify-proof.tsx`

```javascript
const aptosConfig = new AptosConfig({ network: Network.TESTNET }); // Replace witht Network.MAINNET for mainnet
```
